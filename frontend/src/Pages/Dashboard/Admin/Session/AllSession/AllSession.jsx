...
export default function ShowClasses() {
  ...
  const filteredClasses = classes
    .filter(classItem => classItem.session_year?.toLowerCase().includes(searchTerm.toLowerCase()))
    .sort((a, b) => {
      const nameA = a.session_year || '';  // Fallback to empty string if session_year is undefined
      const nameB = b.session_year || '';
    ...
    <div style={{ height: '100%', padding: '20px', backgroundColor: "#f6f7fb", overflow: "auto" }}>
      <div className="classes-container">
        <div className="header">
          <h6>
            Sessions <span className="sub-header"><AiOutlineHome className="sidebar-icon" />- All Sessions</span>
          </h6>
        </div>

        {/* Search Bar */}
        <div className="container-fluid admission-header text-center" style={{ marginTop: '30px' }}>
          <h1>Sessions Record</h1>
        </div>
            ...
            <tbody>
              {filteredClasses.map((classItem, index) => (
                <tr key={classItem._id}>
                  ...
                  <td>{classItem.session_year}</td> {/* Session Year */}
                  <td>{new Date(classItem.start_date).toLocaleDateString()}</td> {/* Start Date */}
      ...
      if (result.isConfirmed) {
        ...
        SessionManager.deleteSession(id)
          .then((res) => {
            ...
            Swal.fire({
              title: "Deleted!",
              text: "Session been deleted successfully.",
              icon: "success",
              confirmButtonColor: "#3085d6"
            });
